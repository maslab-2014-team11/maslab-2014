...
public class BotClient {
	...
	private final Set<BotClientListener> listeners;
	...
	public BotClient(String host_and_port, String token, boolean debug) {
		...
		listeners = new HashSet<BotClientListener>();
	}
	...
	public void sendImage(BufferedImage img) {
		try {
			...
			send("{\"token\":\"" + token + "\",\"IMAGE_DATA\":\"" + result
					+ "\"}done");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String getMap() {
		...
		if (map == null) {
			...
			System.err
					.println("ERROR: MAP NOT RECEIVED FROM BOT_CLIENT. REDEEM THIS LOG MESSAGE FOR A FREE REMATCH. RESTRICTIONS MAY APPLY, ASK YOUR LOCAL MASLAB DEALER FOR MORE INFORMATION.");
			System.exit(-1);
		}
	...
	public class WritebackHandler {
		public void pong() {
			...
			if (debug)
				System.out.println("[CALLBACK] BotClient ping received : "
						+ (ping_end - ping_start) + "ms");
		...
		public void connected() {
			if (debug)
				System.out
						.println("[CALLBACK] BotClient connected successfully!");
		}
		...
		public void map(String m) {
			...
			map = m;
			synchronized (listeners) {
				for (BotClientListener l : listeners) {
					l.onReceiveMap(m);
				}
			}
		}
